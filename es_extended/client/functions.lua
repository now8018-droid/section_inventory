...
function ESX.SearchInventory(items, count)
    if not ESX.PlayerData.inventoryByName then
        rebuildInventoryByName()
    end

    local inventoryByName = ESX.PlayerData.inventoryByName
    if type(items) == 'string' then
        local item = inventoryByName[items]
        if not item then return count and 0 or nil end
        return count and item.count or item
    end

    local data = {}
    for i = 1, #items do
        local itemName = items[i]
        local item = inventoryByName[itemName]
        if item then
            data[itemName] = count and item.count or item
        end
    end
...
function ESX.UI.ShowInventoryItemNotification(add, item, count)
    -- ไม่มี NUI กระเป๋า ESX — ให้กระเป๋าคัสตอมฟัง event นี้ถ้าต้องการแจ้งเตือน
    TriggerEvent("esx:inventoryItemNotification", add, item, count)
end
...
function ESX.Game.GetPlayers(onlyOtherPlayers, returnKeyValue, returnPeds)
    ...
    if now - lastPlayerUpdate > 500 then
        local active = GetActivePlayers()
        local newCache = {}
        for i = 1, #active do
            local player = active[i]
            newCache[#newCache + 1] = { id = player, ped = GetPlayerPed(player) }
        end
...
function ESX.Game.GetClosestObject(coords, modelFilter)
    if not modelFilter then
        local vector = type(coords) == "vector3" and coords or (coords and vec(coords.x, coords.y, coords.z))
        if not vector then
            vector = GetEntityCoords(ESX.PlayerData.ped)
        end
        local object = GetClosestObjectOfType(vector.x, vector.y, vector.z, 50.0, 0, false, false, false)
        if DoesEntityExist(object) then
            return object, #(vector - GetEntityCoords(object))
        end
    end
...
function ESX.Game.GetClosestPlayer(coords)
    local players = ESX.Game.GetPlayers(true, true)
    ...
    for playerId, ped in pairs(players) do
        local dist = #(vector - GetEntityCoords(ped))
...
function ESX.Game.GetClosestVehicle(coords, modelFilter)
    if not modelFilter then
        ...
        local vehicle = GetClosestVehicle(vector.x, vector.y, vector.z, 100.0, 0, 70)
        if DoesEntityExist(vehicle) then
            return vehicle, #(vector - GetEntityCoords(vehicle))
        end
    end
...
function ESX.GetAccount(account)
    local idx = ESX.PlayerData.accountsByName and ESX.PlayerData.accountsByName[account]
    if idx then
        return ESX.PlayerData.accounts[idx]
    end
...
function ESX.ShowInventory()
    -- กระเป๋าเริ่มต้น ESX / esx_inventory ถูกปิด — ให้สคริปต์กระเป๋าของคุณเปิด UI เอง (หรือ export ของคุณ)
    TriggerEvent("esx:requestOpenInventory")
end
...
function ESX.GetVehicleTypeClient(model)
    ...
    return vehicleClassTypes[vehicleType] or "automobile"
end
